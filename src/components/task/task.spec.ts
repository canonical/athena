...
test("dashboard shows task history and start chat button", async ({ page }) => {
  await authenticate(page);
  const loop = await createLoop(page, `Loop with dashboard ${Date.now()}`);
...
test("start chat button opens chat drawer", async ({ page }) => {
  await authenticate(page);
  const loop = await createLoop(page, `Loop chat drawer test ${Date.now()}`);
...
test("send queues routing decision immediately when in routing phase", async ({ page }) => {
  await authenticate(page);
  const loop = await createLoop(page, `Loop queued chat complete ${Date.now()}`);

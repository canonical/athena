...
export function ProviderEditor({ provider, onSuccess, onDelete, isDeleting = false }: ProviderEditorProps) {
  ...
  const parseProviderFormValues = (values: ProviderFormValues) => {
    const payload = {
      ...
      chatDefaultModel: provider?.chatDefaultModel,
      chatEnabledModels: provider?.chatEnabledModels,
      embeddingDefaultModel: provider?.embeddingDefaultModel,
      embeddingEnabledModels: provider?.embeddingEnabledModels,

...
const getEncryptionKey = (): Buffer => {
  if (!backendConfig.credentials.encryptionKey) {
    throw new Error(`Credential encryption key is required for encrypted credentials.`);
  }

  return createHash(`sha256`).update(backendConfig.credentials.encryptionKey).digest();
};

...
Athena should maintain a registered runner type catalog with per-entry lifecycle state. A harness definition record stores `type` to identify which execution environment it connects to.
...
- Provider endpoints accept HTTP and HTTPS.

...
A webhook task is an task triggered by an inbound webhook payload from an external system configured for a loop. Webhook ingestion is optional and must be configured per loop.
...
- When a webhook payload is received, Athena creates a task in the loop with the payload as context and no assigned persona.
...
The webhook payload is normalized into task context by Athena before being passed to the active routing persona. The context includes:

- The originating system and task type.
- The payload body, normalized to the task context format defined in [task.md](./task.md).
...
flowchart TD
	...
	E -->|Yes| F[Normalize payload into task context]
	F --> G[Append per-webhook instructions]
	G --> H{Duplicate delivery in dedupe window?}
	H -->|Yes| R202A[Return 202, no new task]
	H -->|No| I[Create unassigned loop task]
	I --> J[Route task to routing persona isRouting true]

...
A user request should include enough context for the engineering manager persona to assign the event and select the appropriate execution persona. Recommended content:
...
- The engineering manager persona responds to each event interactively, and its replies appear as chat messages in the UI.
- If the engineering manager persona needs input or output from another persona, it spawns a new event internally. The result is fed back to the engineering manager persona, which then continues the chat reply. Internal spawned events are not shown as separate chat entries.
...
2. Athena routes this event to the engineering manager persona.
3. The engineering manager persona responds with a list of available actions for the current chat context (for example: Implement, Take action, Create a plan).
...
5. When the user selects or enters an action, a new event is triggered in the loop with that action as context. The standard routing flow applies: Athena routes the event to the engineering manager persona for assignment.

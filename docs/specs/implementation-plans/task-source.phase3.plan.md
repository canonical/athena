# Task Source Phase 3 Plan — Webhook Task

## Overview

This plan covers the implementation of webhook ingestion for Athena loops. Each loop can have multiple webhook configurations. Each inbound webhook payload creates a new task in the loop and follows the standard Athena routing flow described in [theloop.md](../definitions/theloop.md).
...
| `instructions` | text | Per-webhook instructions appended to EM context for tasks from this webhook |
...
6. Normalise the payload into Athena task context (originating system, task type, payload body).
7. Load per-webhook `instructions` and append them to the task context for the engineering manager persona.
8. Extract configured idempotency key (for example provider delivery ID header). If already processed within `dedupeTtlSeconds`, return `202` and skip task creation.
9. Create a new task in the associated loop with no assigned persona and the normalised payload plus webhook instructions as context.
10. Athena routes the task to the engineering manager persona for assignment.
11. Return `202 Accepted` immediately. All task processing is asynchronous.
...
- Validate and sanitise the normalised task context before inserting into the database.
...
- Duplicate deliveries return `202` without creating new tasks.
...
- Per-webhook instructions are included in the context provided to the engineering manager persona for tasks from that webhook.
...
- Duplicate deliveries are ignored through per-webhook idempotency strategy and do not create duplicate tasks.
- Payloads with invalid signatures are rejected and never create tasks.
...
- Accepted payloads result in a loop task routed to the engineering manager persona.
- The inbound endpoint returns `202` immediately; task processing is asynchronous.

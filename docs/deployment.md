...
- `athena` serves the frontend and HTTP API and owns startup migrations when they are not
  managed externally.
- `athena-worker` runs `pg-boss` background jobs and never runs migrations.

The two applications reach PostgreSQL through separate PgBouncer applications. `athena` uses a
session-mode PgBouncer. `athena-worker` uses its own PgBouncer in transaction mode, so `pg-boss`
jobs share server connections instead of holding one per worker client.
...
default privileges apply to newly created objects. Only the web application installs or upgrades
those schemas: Athena migrations run from the web charm or the external migration workflow, and
the web process installs or upgrades `pgboss` when it starts. The worker waits for the grant handshake before

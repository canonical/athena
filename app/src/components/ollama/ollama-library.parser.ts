...
export type OllamaLibraryIndexEntry = {
  slug: string;
  name: string;
  href: string;
  summary: string | null;
...
const toNullableRawText = (value: string | null | undefined) => {
  const trimmedValue = value?.trim() ?? ``;

  return trimmedValue || null;
};

const toAbsoluteLibraryUrl = (path: string) => new URL(path, OLLAMA_LIBRARY_URL).toString();

const unique = (values: string[]) => Array.from(new Set(values));

const findSection = ($: CheerioAPI, headingText: string) => {
...
const parseTagCount = (value: string | null) => {
  if (!value) {
    return null;
  }
...
const parseApplications = ($: CheerioAPI) => {
...
const parseVariants = ($: CheerioAPI) => {
...
const parseReadmeMarkdown = ($: CheerioAPI) => toNullableRawText($(`#editor`).text());

const parseLicense = ($: CheerioAPI) => {
  const heading = $(`h2`)
    .toArray()
 ...
 */
export const parseOllamaLibraryIndex = (html: string): OllamaLibraryIndexEntry[] => {
  const $ = load(html);
 ...
 */
export const parseOllamaLibraryDetailPage = (html: string, indexEntry: OllamaLibraryIndexEntry): OllamaLibraryModel => {
  const $ = load(html);

CREATE TABLE IF NOT EXISTS "loopRunner" (
  ...
  PRIMARY KEY ("loop", "runner")
);

CREATE UNIQUE INDEX IF NOT EXISTS "idxLoopRunnerLoopPriority" ON "loopRunner"("loop", "priority");
CREATE UNIQUE INDEX IF NOT EXISTS "idxLoopRunnerLoopPriorityOverride" ON "loopRunner"("loop", "priorityOverride") WHERE "priorityOverride" IS NOT NULL;
CREATE INDEX IF NOT EXISTS "idxLoopRunnerRunner" ON "loopRunner"("runner");

SELECT ensureUpdatedAtTrigger('loopRunner');

SELECT format('GRANT SELECT, INSERT, UPDATE, DELETE ON %I TO %I', 'loopRunner', :'APP_ROLE_NAME')
\gexec